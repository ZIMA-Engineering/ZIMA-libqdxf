...
public:
    ...
    std::string toUtf8String(std::string t) {
        return decoder.toUtf8(t);
    }
    ...
    unsigned long long int getInt64() {
        return int64;
    }
    bool getBool() {
        return (intData==0) ? false : true;
    }
    int getVersion() {
    ...
    void setCodePage(std::string *c) {
        decoder.setCodePage(c);
    }
    std::string getCodePage() {
        return decoder.getCodePage();
    }
#ifdef DRW_DBG
    int count;//DBG
#endif
